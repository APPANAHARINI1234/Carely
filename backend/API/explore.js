...
module.exports = function (exploreCollection, healthTipsCollection) {
    ...
    router.get("/", async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, message: "Error fetching diseases", error });
        }
    });
    ...
    router.get("/:id", async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, message: "Error fetching disease details", error });
        }
    });
    ...
    router.get("/healthTips/random", async (req, res) => {
        try {
            // ✅ Use MongoDB aggregation to get a random health tip
            ...
            if (randomTip.length > 0) {
                ...
                res.json({ success: false, tip: "No health tips available at the moment." });
            }
        } catch (error) {
            console.error("Error fetching health tip:", error);
            res.status(500).json({ success: false, message: "Failed to fetch health tip", error });
        }
    });
